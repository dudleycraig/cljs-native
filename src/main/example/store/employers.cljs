...
(s/def ::contract :example.store.contracts/key)
(s/def ::developer-role :example.store.developer-roles/key)
(s/def ::from string?)
(s/def ::to string?)
(s/def ::duration (s/keys :from :to))
(s/def ::projects (s/coll-of :example.store.projects/key))
